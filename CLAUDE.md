...
- **Branching**: work targets `main`; the `dev` integration branch was dropped (`478d266`, PR #38) along with its `dev-pr.yml` workflow, and Dependabot plus `.tituskirch-skills.json` (`pr.base`) now point at `main`. `main` is protected by rules `pull_request`, `required_signatures`, `non_fast_forward`, `deletion`. The owner can bypass them (a direct push lands with a "Bypassed rule violations" notice), but default to a branch + PR unless told otherwise — and never bypass signing.
